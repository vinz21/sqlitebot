...
*/
        ...
        if (chosenNavigationPoint == null) {
            ...
            try { setNavpoint("navlog"); } catch (Exception K) { System.out.println("Hello World!"); }
...
log.info("debug1");
         ...
         return;
          ...
          //int navSize = memory.getKnownNavPoints().size()-1;
          int navSize = 130;
          
          if (navCount == navSize) { navCount = 128; }
          ...
          for (int i=0; i<=155; i++) {
            ...
            //for some reason .concat messes with memory(repeats same navpoint)
            //if (testNavigationPoint.isVisibile()) { navVisList.concat(i+","); }
            //if (testNavigationPoint.isVisible()) { navVisList = navVisList+"("+i+")"+testNavigationPoint.UnrealID+","; }
             if (testNavigationPoint.isVisible()) { log.info("navVis("+i+")"+testNavigationPoint.UnrealID); }
            //if (testNavigationPoint.UnrealID.equals("DM-1on1-Crash.InventorySpot3")) {
              //log.info("navVis("+i+")"+testNavigationPoint.UnrealID);
            //}
          }
          
          //log.info("navVisList:"+navVisList);
