...
void ScoutManager::update()
{
  ...
  if (needMoreScouts())
  {
    requestScout(/* bid = */ 150); // Bid 30. JTC - 150 should get high bid
  }
  else
  {
...
void ScoutManager::onRemoveUnit(BWAPI::Unit* unit)
{
  if (scouts.find(unit) != scouts.end())
  {
    ...
    scouts.erase(unit);
    //JTC scouts.erase(scouts.find(unit));
	BWAPI::Broodwar->sendText("scoutReassign");
	desiredScoutCount++;
  }
}
...
{
  //BWAPI::Broodwar->sendText("scout:%d:%d",desiredScoutCount,scouts.size());
...
void ScoutManager::requestScout(double bid)
{
  ...
  //JTC std::set<BWAPI::Unit*> usefulUnits=SelectAll()(isWorker,Overlord)(isCompleted).not(isCarryingMinerals,isCarryingGas,isGatheringGas);
  std::set<BWAPI::Unit*> usefulUnits=SelectAll()(isWorker,Overlord)(isCompleted);
